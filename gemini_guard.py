...
async def ask_gemini(contents, system=None, model_name="gemini-2.5-flash", history=None):
    """
    Send a message to Gemini, optionally with conversation history.

    Parameters
    ----------
    contents : list
        The new message parts (str and/or PIL.Image objects).
    system : str | None
    ...
    history : list[dict] | None
        ...
        Pass None or [] for a fresh conversation (no memory).
    """
    ...
    async with _lock:
        ...
        # 2. Rolling 30-second window (max 10 requests)
        now = time.time()
        _request_history = [t for t in _request_history if now - t < 30]
        if len(_request_history) >= 10:
            print(f"⚠️ Gemini Guard: Rate-limiting ({len(_request_history)} in 30s)")
    ...
    try:
        ...
        if history:
            # Multi-turn: start a chat session with existing history, then send new message
            # Gemini SDK expects history as list of Content dicts with "role" and "parts"
            # Parts must be strings (or PIL images) — we keep it simple here
